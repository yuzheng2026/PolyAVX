以下是一份**完整测试代码**，涵盖了多项式全家桶的所有功能，包括基本代数、微积分、形式幂级数、三角函数/反三角函数、双曲/反双曲函数以及全部扩展运算（平移、插值、复合、复合逆、特殊函数）。  
将代码保存为 `test_full.cpp`，与 `poly_avx.hpp` 放在同一目录，使用推荐编译命令即可运行。
...
### 编译与运行
```bash
g++ -O3 -march=native -mfma -std=c++98 test_full.cpp -o test_full
./test_full
```

### 预期输出
所有恒等式误差应达到前文给出的量级，扩展功能输出正确（平移、插值零误差，复合逆正确）。  
这份测试代码将完全验证多项式全家桶的正确性与高精度。
