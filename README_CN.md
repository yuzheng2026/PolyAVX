...
- `PolyI` —— 使用 `long long` 系数，基于数论变换（NTT）的**精确、确定**的乘法（模素数 `998244353`）。
- 支持 `mul`、`inv`、`log`、`exp`、`sqrt`、`pow`，无浮点误差。
- 额外提供 `mul_exact` 函数，采用**多模数 CRT**（三个 NTT 素数，乘积约 4.7×10²⁶），
  在系数可能超出单个模数范围时进行精确乘法。
- 适合区块链有用工作证明（PoUW）及任何需要可复现结果的场景。
...
测试条件：多项式截断长度 `n=8`，每个函数重复 10000 次，使用高精度计时器。

| 函数 | Windows (µs) | Linux (µs) |
|------|--------------|------------|
| `A + B` | 0.0586 | 0.0433 |
| `A - B` | 0.0512 | 0.0250 |
| `A * B` | 0.1578 | 0.1008 |
...
**注：** Windows 测试使用 MinGW-w64 GCC 16；Linux 测试使用 WSL2 环境下的 GCC 16。两者均使用 `-O3 -march=native -mfma` 编译。
