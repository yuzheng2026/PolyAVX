...
// 辅助：计算最大误差（与期望值比较）
double max_error(const PolyD& p, const std::vector<double>& expected) {
	double err = 0.0;
	for (size_t i = 0; i < expected.size() && i < (size_t)p.size(); ++i) {
		double diff = std::abs(p[i] - expected[i]);
...
// 辅助：计算多项式与零的误差（首项可为1）
double inf_norm(const PolyD& p, bool first_is_one = true) {
	double err = 0.0;
	for (int i = 0; i < p.size(); ++i) {
		double target = (i == 0 && first_is_one) ? 1.0 : 0.0;
...
int main() {
	...
	err = max_error(comp.trunc(std::min(5, A.size())), A.data);
